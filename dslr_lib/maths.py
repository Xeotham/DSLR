from pandas import DataFrame
from numpy import sum, var

def total_count(df: DataFrame):
    return df.shape[0]

def count(df: DataFrame):
    df_nona = df[df.notna()]
    return df_nona.shape[0]

def mean(df: DataFrame):
    df_nona = df[df.notna()]
    return sum(df_nona.values) / count(df_nona)

def var(df: DataFrame):
    df_nona = df[df.notna()]
    df_mean = mean(df)
    return sum([(i - df_mean) ** 2 for i in df_nona.values]) / count(df_nona)

def std(df: DataFrame):
    return var(df) ** 0.5

def min(df: DataFrame):
    df_nona = df[df.notna()]
    df_min = float("inf")
    for i in df_nona.values:
        if i < df_min:
            df_min = i
    return df_min

def max(df: DataFrame):
    df_nona = df[df.notna()]
    df_max = float("-inf")
    for i in df_nona.values:
        if i > df_max:
            df_max = i
    return df_max

def test_quartile(Q: float, arg_list: list) -> float:
    """
    Test and return the result of the found quartile.
    :param Q: Quartile found.
    :param arg_list: list of arguments.
    :return: Value of the quartile.
    """

    Rinf = arg_list[Q.__floor__()]
    Rsup = arg_list[Q.__ceil__()]

    final_value = 0

    if not Q.is_integer():
        if Q % 1 == 0.25:
            final_value = ((Rinf * 3) + Rsup) / 4
        elif Q % 1 == 0.5:
            final_value = (Rinf + Rsup) / 2
        elif Q % 1 == 0.75:
            final_value = (Rinf + (Rsup * 3)) / 4
    else:
        final_value = arg_list[Q.__floor__()]

    return final_value

def Q1(df: DataFrame):
    df_nona = df[df.notna()].sort_values()
    df_q1 = ((len(df_nona) + 3) / 4)

    return test_quartile(df_q1 - 1, df_nona.to_list())

def Q2(df: DataFrame):
    df_nona = df[df.notna()].sort_values()
    df_q2 = (((len(df_nona)) + 1) / 2)

    return test_quartile(df_q2 - 1, df_nona.to_list())

def Q3(df: DataFrame):
    df_nona = df[df.notna()].sort_values()
    df_q3 = (((3 * len(df_nona)) + 1) / 4)

    return test_quartile(df_q3 - 1, df_nona.to_list())

def nan_count(df: DataFrame):
    return sum(1 for x in df if x != x)
